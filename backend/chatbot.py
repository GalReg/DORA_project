...
df["combined_info"] = df.apply(
    lambda row: f"Order time: {row['created_at']}. Customer Name: {row['name']}. Product Department: {row['product_department']}. Product: {row['product_name']}. Category : {row['product_category']}. Price: ${row['sale_price']}. Stock quantity: {row['stock_quantity']}",
    axis=1,
)
...
qa = RetrievalQA.from_chain_type(
    ...
    verbose=True,
    chain_type_kwargs={"verbose": True, "prompt": chatbot_prompt, "memory": memory},
)
